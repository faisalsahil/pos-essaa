class InvoiceBlurbsController < ApplicationController
  ...
  def index
    @invoice_blurbs = @current_vendor.invoice_blurbs.page(params[:page]).per(@current_vendor.pagination)
  end

  def show
    @invoice_blurb = @current_vendor.invoice_blurbs.find_by_id(params[:id])
