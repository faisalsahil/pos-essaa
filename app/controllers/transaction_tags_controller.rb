...
class TransactionTagsController < ApplicationController
  ...
  def index
    @transaction_tags = @current_vendor.transaction_tags.page(params[:page]).per(@current_vendor.pagination).order('created_at DESC')
  end

  def show
    @transaction_tag = @current_vendor.transaction_tags.find_by_id(params[:id])
