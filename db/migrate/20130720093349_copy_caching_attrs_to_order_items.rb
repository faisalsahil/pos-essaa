class CopyCachingAttrsToOrderItems < ActiveRecord::Migration
  def up
    ...
    Vendor.connection.execute("UPDATE order_items SET is_quote = orders.is_quote, is_unpaid = orders.is_unpaid, paid = orders.paid, paid_at = orders.paid_at, created_at=orders.created_at, completed_at=orders.completed_at from orders WHERE order_items.order_id = orders.id")
