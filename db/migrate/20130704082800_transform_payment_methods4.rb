class TransformPaymentMethods4 < ActiveRecord::Migration
  def up
    ...
    Vendor.connection.execute("UPDATE payment_method_items SET payment_method_id = payment_methods.id from payment_methods WHERE payment_methods.name LIKE CONCAT( SUBSTRING(payment_method_items.internal_type, 1, 4), '%')")
